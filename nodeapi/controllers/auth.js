...
//    const user = await new User(req.body);
//    await user.save();
//    res.status(200).json({ message: "Signup success! Welcome aboard! And have fun!" });
//  };
...
//      if(err || !user) {
//        return res.status(401).json({
//          error: 'User with that email does not exist. Sign in, if you would please!'
//        });
//      }
    // if user is found, make sure the email and password match
    // creaete authenticate method in model and use here
//      if(!user.authenticate(password)) {
//        return res.status(401).json({
//          error: 'Email and password do not seem to be matching!'
//        });
//      }
    // generate a token with user id and secret
//      const token = jwt.sign({ _id: user._id }, process.env.JWT_SECRET);
    //persist the token as 't' in cookie with expiry Date
//      res.cookie('t', token, { expire: new Date() + 9999 });
    //return response with user and toekn to frontend client
//      const { _id, name, email } = user;
//      return res.json({ token, user: { _id, email, name } });
//    });
//  };
...
//  exports.signout = (req, res) => {
//    res.clearCookie('t');
//    return res.json({ message: 'Signout successful! Enjoy the rest of your day!' });
//  };
